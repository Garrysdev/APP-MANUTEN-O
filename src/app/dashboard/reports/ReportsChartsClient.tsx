...
import { TIPO_LABELS, type TipoTarefa } from '@/types/models'
...
export default function ReportsChartsClient({
  ...
  const monthlyData = useMemo(() => {
    ...
    return months.map((m, i) => {
      // Simulação calculada realista baseada nas OTs existentes
    ...
    <div className="space-y-8 my-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
          ...
          <div className="h-56 flex items-end justify-between gap-3 pt-6 px-2 border-b border-slate-200 dark:border-slate-800">
            {monthlyData.map((d) => {
              const maxVal = 15
              const hReq = Math.round((d.piRequested / maxVal) * 100)
              const hComp = Math.round((d.piCompleted / maxVal) * 100)
              return (
                <div key={d.month} className="flex-1 flex flex-col items-center gap-2 h-full justify-end group">
                  <div className="w-full flex items-end justify-center gap-1.5 h-full">
                    <div
                      style={{ height: `${hReq}%` }}
                      className="w-1/2 bg-blue-500 rounded-t transition-all group-hover:bg-blue-600 relative"
                      title={`Solicitados: ${d.piRequested}`}
                    >
                      <span className="opacity-0 group-hover:opacity-100 absolute -top-5 left-1/2 -translate-x-1/2 text-[10px] font-bold bg-slate-800 text-white px-1 rounded transition-opacity">
                        {d.piRequested}
                      </span>
                    </div>
                    <div
        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 shadow-sm">
          ...
          <div className="h-56 flex items-end justify-between gap-3 pt-6 px-2 border-b border-slate-200 dark:border-slate-800">
            {monthlyData.map((d) => {
              const height = Math.round(d.pmCompliance)
              return (
                <div key={d.month} className="flex-1 flex flex-col items-center gap-2 h-full justify-end group">
                  <div className="w-full flex items-end justify-center h-full">
                    <div
                      style={{ height: `${height}%` }}
                      className={`w-3/4 rounded-t transition-all relative ${
                        d.pmCompliance >= 95 ? 'bg-emerald-500' : d.pmCompliance >= 90 ? 'bg-blue-500' : 'bg-amber-500'
                      }`}
                    >
                      <span className="opacity-0 group-hover:opacity-100 absolute -top-5 left-1/2 -translate-x-1/2 text-[10px] font-bold bg-slate-800 text-white px-1 rounded transition-opacity whitespace-nowrap">
                        {d.pmCompliance}%
                      </span>
                    </div>
                  </div>
